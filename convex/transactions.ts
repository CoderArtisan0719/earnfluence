...
export const deposit = mutation({
  args: { clientId: v.id('talents'), amount: v.number() },
...
export const withdraw = mutation({
  args: { userId: v.id('talents'), amount: v.number() },
...
export const updateContract = internalMutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (sender!.type === 0 && args.type === 'refunded') {
        await ctx.db.patch(bid.clientId, {
          balance: JSON.stringify(senderBalance + bid.amount),
        });
      }

      if (receiver!.type === 1 && args.type === 'closed') {
